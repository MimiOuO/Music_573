...
//    self.navigationController.navigationBar.hidden = NO;
    ...
    if (NO == self.navigationController.navigationBar.translucent) {
        ...
        self.searchSuggestionVC.view.frame = CGRectMake(0, NavH - self.view.py_y, self.view.py_width, self.view.py_height + self.view.py_y);
...
#pragma clang diagnostic pop
        ...
        searchSuggestionVC.view.frame = CGRectMake(0, NavH, PYScreenW, PYScreenH);
...
- (void)setup
{
    ...
    UIView *titleView = [[UIView alloc] initWithFrame:frame(Mar, 10, 280, 30)];
    ...
    self.navView.centerButton.hidden = YES;
    [self.navView.rightButton setTitle:@"取消" forState:UIControlStateNormal];
    self.navView.rightButton.titleLabel.font = [UIFont systemFontOfSize:16];
    self.navView.rightButton.left = KSW - 60;
...
- (void)layoutForDemand {
    if (NO == self.swapHotSeachWithSearchHistory) {
        self.hotSearchView.py_y = PYSEARCH_MARGIN * 5;
    ...
    switch (self.searchResultShowMode) {
        ...
        case PYSearchResultShowModeEmbed:
            if (self.searchResultController) {
                ...
                self.searchResultController.view.hidden = NO;
                self.searchResultController.view.py_y = NO == self.navigationController.navigationBar.translucent ? 0 : NavH;
