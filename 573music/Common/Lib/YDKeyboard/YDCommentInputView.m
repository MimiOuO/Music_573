...
static const CGFloat kInputBarHeight = 50;
...
- (void)awakeFromNib
{
    ...
    self.commentInputTextField.backgroundColor = appBgColor;
    ViewRadius(self.commentInputTextField, 18);

    self.commentInputTextField.leftView = [[UIView alloc] initWithFrame:CGRectMake(10 ,10 ,20,20)];
    self.commentInputTextField.leftView.userInteractionEnabled = NO;
    self.commentInputTextField.leftViewMode = UITextFieldViewModeAlways;
    self.commentInputTextField.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
    
    if (KSW > 400) {
        self.commentInputTextField.width = 400;
    }
}
...
//    NSInteger section = indexPath.section;
    ...
    cell.deleteImageView.image = image(@"backspace");
...
- (IBAction)onEmotionButtonAction:(id)sender {
    ...
    if (self.inputBottomViewHeight != collectionViewHeight) {
