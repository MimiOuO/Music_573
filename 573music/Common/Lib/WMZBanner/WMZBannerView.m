...
- (void)setIconData:(UIImageView*)icon withData:(id)data{
    ...
    if ([data isKindOfClass:[NSString class]]) {
        if ([(NSString*)data hasPrefix:@"http"]) {
            [icon sd_setImageWithURL:[NSURL URLWithString:(NSString*)data] placeholderImage:image(@"qxt_logo")];
        }else{
...
- (void)scrollEnd:(NSIndexPath*)indexPath{
    ...
    if (self.param.wEventScrollEnd) {
        ...
        NSInteger index = self.param.wRepeat?(indexPath.row + 2)%self.data.count:indexPath.row;
