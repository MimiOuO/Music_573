...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        swizzleCollectMethod(targetClass, originalSelector, swizzledSelector);
    });
}

void swizzleCollectMethod(Class class, SEL originalSelector, SEL swizzledSelector) {
...
- (void)setFooterShow {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (heightOfContentSize < 440) { // 实际显示高度大于内容高度 代表第一页都没有占满
